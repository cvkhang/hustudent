...
import { Zap, X, Loader2, Library, Globe, Lock } from 'lucide-react';
import ProButton from '@/components/ui/ProButton';
import { toast } from 'sonner';

const SetModal = ({ isOpen, onClose, onSubmit, initialData = null, isPending, title = "Tạo bộ thẻ mới" }) => {
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-black text-slate-800 flex items-center gap-3">
              <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-yellow-50 text-yellow-500">
                <Zap size={20} className="stroke-[2.5]" fill="currentColor" />
              </span>
              {title}
            </h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1">Tên bộ thẻ <span className="text-red-500">*</span></label>
            <input
              required
              className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 font-bold text-slate-700 transition-colors shadow-sm"
          ...
          <div>
            ...
            <textarea
              className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 font-medium text-slate-700 transition-colors resize-none h-24 shadow-sm"
          ...
          <div className="pt-2 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 sm:items-center">
            {initialData?.id && (
              <ProButton
                ...
                onClick={() => {
                  onClose();
                  navigate(`/flashcards/${initialData.id}`);
                }}
              >
                Quản lý thẻ
              </ProButton>
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              ...
              <ProButton type="submit" variant="primary" disabled={isPending} isLoading={isPending} className="flex-1 sm:flex-none shadow-lg shadow-yellow-200 bg-yellow-500 hover:bg-yellow-600 border-yellow-500 min-w-[120px] whitespace-nowrap !px-6">
